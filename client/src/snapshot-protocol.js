(() => {
  ...
  async function activateExecutorMode(token) {
    ...
    window.addEventListener("message", async (event) => {
      ...
      if (!message || message.type !== "snapshot:navigate") {
        return;
      }
      ...
      if (message.sessionToken !== token || typeof message.requestId !== "string") {
        return;
      }
      ...
      if (!targetPath) {
        postToExecutor({
          ...
          code: "INVALID_ROUTE",
          message: "The executor supplied an invalid target path."
        });
        return;
      }
      ...
      try {
        let readiness;
        const canUseStartupReadiness =
          firstRequest &&
          startupReadiness?.isConnected === true &&
    ...
    // The script being loaded does not prove the SPA router is ready. The
    // application's initial semantic readiness signal is the protocol handshake.
    startupReadiness = await waitForReadiness(null);
    window.__snapshotProtocolReady = true;
    postToExecutor({
      type: "snapshot:client-ready",
      sessionToken: token,
      protocolVersion: PROTOCOL_VERSION,
      clientVersion: CLIENT_VERSION
    });
  ...
  function markCurrentReadinessConsumed() {
    document.querySelectorAll(READY_ELEMENT).forEach((element) => {
  ...
  function waitForReadiness(timeoutMs) {
    return new Promise((resolve, reject) => {
      ...
      observer.observe(document.documentElement, {
        ...
        subtree: true,
        attributes: true,
        attributeFilter: [PROCESSED_ATTRIBUTE]
      });
    });
  }
  ...
  function transformSnapshot(html, capturedRoute) {
    ...
    if (!readiness) {
      throw new Error(`Captured document did not contain <${READY_ELEMENT}>.`);
    }
  ...
  function validateSnapshotVersion() {
    ...
    fetch("/index.html", { cache: "no-store", signal: controller.signal })
      .then((response) => {
        if (!response.ok) {
          throw new Error(`Root index returned HTTP ${response.status}.`);
        }
      ...
      .then((html) => {
        ...
        if (!rootScript) {
          console.warn(`${logPrefix} Root index.html does not contain #${SCRIPT_ID}; version validation was skipped.`);
          return;
        }
      ...
      .catch((error) => {
        if (error?.name !== "AbortError") {
          console.warn(`${logPrefix} Snapshot version validation failed; hydration will continue.`, error);
        }
      })
  ...
  function getSiteVersion(element) {
    return element?.getAttribute("data-site-version")?.trim() || null;
  }

  function postToExecutor(message) {
