...
internal sealed class BrowserWorker : IAsyncDisposable
{
    ...
    public async Task<SnapshotRenderResult> RenderAsync(SnapshotRoute route, int maximumAttempts, CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= maximumAttempts; attempt++)
        {
            ...
            {
                await ResetPageAsync(cancellationToken).ConfigureAwait(false);
            }
            else if (attempt > 1)
            {
                await ResetPageAsync(cancellationToken).ConfigureAwait(false);
            ...
            try
            {
                ...
                if (envelope.Type == SnapshotProtocolConstants.ResultMessage && envelope.Html is not null)
                {
                    ...
                    {
                        stopwatch.Stop();
                        return new SnapshotRenderResult(
                            route,
                            true,
                            envelope.Html,
                            attempt,
            ...
            _logger.Log(new SnapshotLogEntry(
                ...
                $"Worker {_workerId} failed {route.Path} on attempt {attempt}/{maximumAttempts}: {lastMessage}"));
        }
        ...
        return new SnapshotRenderResult(
            ...
            stopwatch.Elapsed,
            lastCode ?? SnapshotDiagnosticCodes.BrowserFailure,
            lastMessage ?? "Snapshot rendering failed.",
            _consoleErrors.ToArray(),
            _failedRequests.ToArray());
    }
    ...
    private async Task ResetPageAsync(CancellationToken cancellationToken)
    {
        ...
        await _page.GotoAsync(activationUri.ToString(), new PageGotoOptions
        {
            WaitUntil = WaitUntilState.DOMContentLoaded,
            Timeout = (float)_timeouts.StartupTimeout.TotalMilliseconds
        }).ConfigureAwait(false);
        await _page.WaitForFunctionAsync(
            "() => window.__snapshotProtocolReady === true",
            null,
            new PageWaitForFunctionOptions { Timeout = (float)_timeouts.StartupTimeout.TotalMilliseconds }).ConfigureAwait(false);
        cancellationToken.ThrowIfCancellationRequested();
    }
    ...
    private void Receive(string payload)
    {
        try
        {
            ...
            if (envelope?.RequestId is not null &&
                string.Equals(envelope.SessionToken, _sessionToken, StringComparison.Ordinal) &&
