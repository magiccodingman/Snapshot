...
public sealed class PlaywrightBrowserInstaller
{
    ...
    public PlaywrightBrowserInstaller(ISnapshotLogger logger) => _logger = logger;
    ...
    public async Task EnsureInstalledAsync(PlaywrightSnapshotOptions options, CancellationToken cancellationToken)
    {
        ...
        catch (Exception exception) when (exception is not OperationCanceledException && options.BrowserInstallMode == BrowserInstallMode.InstallIfMissing)
        {
            ...
            _logger.Log(new SnapshotLogEntry(SnapshotLogLevel.Warning, "Chromium was missing or failed launch validation; installing the matching Playwright browser.", exception));
        }
        ...
        {
            TryDeleteMarker(GetMarkerPath());
            throw new InvalidOperationException("A complete matching Chromium installation was not found. Run 'snapshot browser install' or use InstallIfMissing mode.", exception);
        }
    ...
    private async Task InstallCoreAsync(bool withDependencies, CancellationToken cancellationToken)
    {
        ...
        var arguments = withDependencies && OperatingSystem.IsLinux() ? new[] { "install", "--with-deps", "chromium" } : new[] { "install", "chromium" };
        ...
        cancellationToken.ThrowIfCancellationRequested();
        if (exitCode != 0) throw new InvalidOperationException($"Playwright browser installation failed with exit code {exitCode}.");
    ...
    private static async Task WriteMarkerAsync(CancellationToken cancellationToken)
    {
        ...
        await File.WriteAllTextAsync(markerPath, $"validated={DateTimeOffset.UtcNow:O}{Environment.NewLine}playwright={GetPlaywrightVersion()}{Environment.NewLine}platform={GetPlatformKey()}{Environment.NewLine}", cancellationToken).ConfigureAwait(false);
    }

    private static string GetMarkerPath()
    {
        var basePath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
        if (string.IsNullOrWhiteSpace(basePath)) basePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".snapshot");
        return Path.Combine(basePath, "Snapshot", "Playwright", GetPlaywrightVersion(), GetPlatformKey(), "chromium.validated");
    }

    private static string GetPlaywrightVersion() => typeof(Microsoft.Playwright.Playwright).Assembly.GetName().Version?.ToString() ?? "unknown";
