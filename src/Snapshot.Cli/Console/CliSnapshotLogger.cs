...
internal sealed class CliSnapshotLogger : ISnapshotLogger
{
    ...
    public CliSnapshotLogger(bool verbose) => _verbose = verbose;

    public void Log(SnapshotLogEntry entry)
    {
        if (entry.Level == SnapshotLogLevel.Trace && !_verbose) return;
        ...
        if (_verbose && entry.Exception is not null) Console.WriteLine(entry.Exception);
