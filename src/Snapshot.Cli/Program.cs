...
internal static class SnapshotCli
{
    ...
    private static async Task<int> BuildAsync(CommandLine input, CliSnapshotLogger logger)
    {
        ...
        var request = new SnapshotBuildRequest
        {
            ...
            Timeouts = new SnapshotTimeoutOptions { RouteTimeout = routeTimeout, StartupTimeout = startupTimeout, WatchdogTimeout = watchdogTimeout },
            Retry = new SnapshotRetryOptions { MaximumAttempts = input.GetInt("retries") ?? 3 },
            Concurrency = input.GetInt("concurrency"),
            Hosting = new SnapshotHostingOptions { Provider = input.Has("netlify") ? SnapshotHostingProvider.Netlify : SnapshotHostingProvider.Generic },
        ...
        Console.WriteLine();
        Console.WriteLine(result.Succeeded ? $"Created {result.OutputPath} in {result.Elapsed}." : $"Snapshot build failed after {result.Elapsed}.");
    ...
    private static async Task<int> ValidateHostAsync(CommandLine input)
    {
        ...
        var manifest = await archive.TryReadManifestAsync().ConfigureAwait(false) ?? throw new InvalidDataException("snapshot-manifest.json is missing.");
    ...
    private static async Task<int> InspectAsync(CommandLine input)
    {
        ...
        await using var archive = await SnapshotArchive.OpenAsync(path).ConfigureAwait(false);
        if (input.Get("file") is { } requestedFile)
        {
            Console.WriteLine(await archive.ReadTextAsync(requestedFile).ConfigureAwait(false));
            return 0;
        }
        ...
        await foreach (var archiveFile in archive.EnumerateFilesAsync().ConfigureAwait(false))
            Console.WriteLine($"{archiveFile.UncompressedLength,12:N0}  {archiveFile.Path}");
        return 0;
    }
    ...
    private static async Task<int> BrowserAsync(CommandLine input, CliSnapshotLogger logger)
    {
        ...
        if (action == "status")
        {
            ...
            Console.WriteLine(valid ? $"Chromium launched successfully. Validation marker: {installer.GetInstallationMarkerPath()}" : "No complete matching Chromium installation could be launched.");
            return valid ? 0 : 3;
        }
    ...
    private static string RequiredPositional(CommandLine input, int index, string message) => input.Positionals.Count > index ? input.Positionals[index] : throw new ArgumentException(message);
